...
/**
 * 这个函数用来构造reference_lines 和 segments
 * @param reference_lines 输出变量
 * @param segments 输出变量
 * */
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  // 更新一下当前的车辆状态
  // 是否是因为vehicle_state_被不断的更改，所以在计算之前，存出一个不会被更改的版本
  common::VehicleState vehicle_state;
  {
    std::lock_guard<std::mutex> lock(vehicle_state_mutex_);
    vehicle_state = vehicle_state_;
  }
  // 同理适用与routing response，保存出一个最新的版本
  routing::RoutingResponse routing;
  {
    std::lock_guard<std::mutex> lock(routing_mutex_);
    routing = routing_;
  }
  // 看这个，判断一下是不是新的不同的routing
  bool is_new_routing = false;
  {
    ...
    if (pnc_map_->IsNewRouting(routing)) {
      is_new_routing = true;
      // 如果是新的routing需要在这里调用pnc map 的函数，更新routing，
      // 这一步会整体调整pncmap中的很多变量
  ...
  // 在这里完成segments的计算
  // 其实内部也是调用的pnc map的接口，也有一些其他的处理，可以仔细看一下

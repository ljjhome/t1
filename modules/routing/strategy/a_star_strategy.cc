...
bool AStarStrategy::Search(const TopoGraph* graph,
  ...
  std::cout << "src node : "<<src_node<<std::endl;
  std::cout << "src node id : "<<src_node->LaneId()<<std::endl;
  std::cout << " dst node : "<<dest_node<<std::endl;
  std::cout << "dst node id : "<<dest_node->LaneId()<<std::endl;
  ...
  while (!open_set_detail.empty()) {
    std::cout <<"count_lanelet : " << ++count_lanelet<<std::endl;
    current_node = open_set_detail.top();
    const auto* from_node = current_node.topo_node;
    std::cout << "current_node.topo_node->LaneId() : "<<current_node.topo_node->LaneId()<<std::endl;
    if (current_node.topo_node->LaneId() == dest_node->LaneId()) {
    ...
    for (const auto* edge : next_edge_set) {
      std::cout << "count_edge : "<< ++count_edge <<std::endl;

...
std::vector<MapPathPoint> MapPathPoint::GetPointsFromLane(LaneInfoConstPtr lane,
  ...
  for (size_t i = 0; i < lane->points().size(); ++i) {
    // 当积累s在【start s ， end s 】区间内时，构造一个point
    ...
    if (i < lane->segments().size()) {
      ...
      // 这个if和下面那个if的含义是，找到跨越start s 和end s 的位置，这样能把这首尾的两个点放进去
...
void Path::InitPoints() {
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    heading.Normalize(); // 求heading的单位向量
    unit_directions_.push_back(heading); // 单位向量作为方向
  }
  length_ = s; //记录长度
  // 均匀化的时候的采样数量

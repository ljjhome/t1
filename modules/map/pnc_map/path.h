...
// 一个laneWayPoint：一个lane 加上一个 在lane 中的s（或者s和l）组成一个point
struct LaneWaypoint {
...
// LaneSegment 是指，一个lane，加上一个start_s 和一个end_s
struct LaneSegment {
  LaneSegment() = default;
...
// 这个PathOverlap，是指一个交通对象id(交通灯，停止线，标志等等) 和 它所占据的位置的start s 和end s
struct PathOverlap {
  PathOverlap() = default;

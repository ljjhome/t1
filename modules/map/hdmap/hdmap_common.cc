...
// 这个是laneinfo的构造，直接把pblane 放进来，
// 然后运行初始化函数
LaneInfo::LaneInfo(const Lane &lane) : lane_(lane) { Init(); }
// 这个就是初始化函数
void LaneInfo::Init() {
  // 把lane 的中心线放到pionts_中
  std::cout << "----------lane id : "<< lane_.id().id()<<"-------"<<std::endl;
  PointsFromCurve(lane_.central_curve(), &points_);
  std::cout << "lane point size : "<<points_.size()<<std::endl;
  ...
  for (size_t i = 0; i + 1 < points_.size(); ++i) {
    ...
    accumulated_s_.push_back(s); // 记录累加的s
    unit_directions_.push_back(segments_.back().unit_direction());// 记录segment的方向向量
    s += segments_.back().length(); // 累加segment的长度就是s
  }
  // 最后一个s放进来
...
// 把HDMapImpl 发进来，在调用处就是this指针传进来

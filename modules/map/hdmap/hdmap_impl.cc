...
int HDMapImpl::LoadMapFromFile(const std::string& map_filename) {
  ...
  // 如果是xml类型的可以直接调用opendriveAdapter的parse
  if (absl::EndsWith(map_filename, ".xml")) {
    if (!adapter::OpendriveAdapter::LoadData(map_filename, &map_)) {
      return -1;
    }
    // 如果是bin或者txt类型，可以直接调用google protobuf的接口进行还原
  } else if (!cyber::common::GetProtoFromFile(map_filename, &map_)) {
    return -1;
  }
  // 上面这些操作都是把一个文件转化成了proto
  // 然后到了下面这一句就是直接把proto变成map数据类型
  // 所以可以有一个结论：我们似乎不需要直到百度的格式spec，我们只要直到proto是怎么被用起来的
  // 我们直接写程序去构造一个proto map ，然后调用后面的接口就完了。
  // 反正proto是开源的

  // 也就是说，只要直到proto的定义和用法，我们就可以直接解析josm，或者直接利用lanelet，写一个proto就行
...
/**
 * 这个函数能指出，各个交通对象在内部是怎么存储的
 * */
int HDMapImpl::LoadMapFromProto(const Map& map_proto) {
  std::cout << "in HDMapImpl::LoadMapFromProto(const Map& map_proto)" <<std::endl;
  // 直接指针，避免copy
  ...
  // 把protoMap中所有的lane取出来，按照id放到一个unordered map中
  for (const auto& lane : map_.lane()) {
    lane_table_[lane.id().id()].reset(new LaneInfo(lane));
  }
  // 同理取出junction放到这个map里面
  // 后面同理，通过一个id和对象的unordered map
  ...
  for (const auto& road_ptr_pair : road_table_) {
    ...
    // 从road当中取出每一个section进行循环
    for (const auto& road_section : road_ptr_pair.second->sections()) {
      // 记录下这个section的id
      const auto& section_id = road_section.id();
      // 然后对road section 中的每一个lane进行循环
      for (const auto& lane_id : road_section.lane_id()) {
        // 到lane table中寻找这个lane
        auto iter = lane_table_.find(lane_id.id());
        if (iter != lane_table_.end()) { // 如果找到了这个lane
          iter->second->set_road_id(road_id); // 就把road id 和section id 放到这个lane中存储起来，方便后面进行操作
          iter->second->set_section_id(section_id);
        } else {
          // 如果没有找到这个lane，说明出问题了，有一个lane没有被记录在地图中
  ...
  for (const auto& lane_ptr_pair : lane_table_) {
    // 对每一个lane 都把目前的这个this指针传进去
    // 在这个函数当中把所有和这个lane相关的对象都添加到了这个lane的成员变量当中
...
int HDMapImpl::GetLanes(const PointENU& point, double distance,
  ...
  std::cout << "in hdmap impl GetLanes with parameters"<<std::endl;
  return GetLanes({point.x(), point.y()}, distance, lanes);
}

int HDMapImpl::GetLanes(const Vec2d& point, double distance,
                        std::vector<LaneInfoConstPtr>* lanes) const {
  // std::cout << " in HDMapImpl::GetLanes with vec2d" <<std::endl;
  bool f1 = lanes == nullptr;
  // std::cout << "after bool f1"<<std::endl;
  bool f2 = lane_segment_kdtree_ == nullptr;
  // std::cout << "after bool f2"<<std::endl;
  if (lanes == nullptr || lane_segment_kdtree_ == nullptr) {
    std::cout << "lanes are empty"<<std::endl;
    return -1;
  }
  // std::cout << " in HDMapImpl::GetLanes with vec2d befroe clear "<<std::endl;
  lanes->clear();
// std::cout << " in HDMapImpl::GetLanes with vec2d afeter clear "<<std::endl;
  std::vector<std::string> ids;
  // std::cout << "in getlanes "<<std::endl;
  const int status =
...
void HDMapImpl::BuildSegmentKDTree(const Table& table,
  ...
  for (const auto& info_with_id : table) {
    ...
    std::cout << "info_with_id : "<<info_with_id.second->id().id()<<std::endl;
    std::cout << "info_with_id size "<<info->segments().size()<<std::endl;
    for (size_t id = 0; id < info->segments().size(); ++id) {
      ...
      box_table->emplace_back(
          ...
          &segment, id);
      // std::cout << "segment.start() : "<<segment.start()<<" , "<<segment.end()<<std::endl;
      std::cout <<"segment.start() : "<<segment.start().x() << " , "<<segment.start().y() << std::endl;
      std::cout <<"segment.end() : "<<segment.end().x() << " , "<<segment.end().y() << std::endl;
    }
  }
...
void HDMapImpl::BuildLaneSegmentKDTree() {
  ...
  std::cout << "build lane seg kdtree"<<std::endl;

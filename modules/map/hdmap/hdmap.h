 ...
 * @class HDMap
 *
 * @brief High-precision map loader interface. // 用来loadmap的？
 * 
 * 这个类里面有一个impl，在impl里面实现了具体的调用方式
 * 这个类就是进行了一层封装
 */
class HDMap {
 public:
  /**
   * @brief load map from local file 从文件load，0表示成功
   ...
   * @return 0:success, otherwise failed
   */
  int LoadMapFromProto(const Map& map_proto); // 从某个特定的protobuf 进行load
